...
function ReplyModal({ lead, onClose }: { lead: RespondedLead; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-label={`Reply from ${lead.name}`}>
      ...
      <div className="relative w-full max-w-2xl max-h-[85vh] flex flex-col bg-[var(--crm-surface-2)] border border-[var(--crm-border-strong)] rounded-2xl shadow-2xl overflow-hidden">
        <div className="flex items-start justify-between gap-3 px-5 py-4 border-b border-[var(--crm-border)] bg-[var(--crm-accent-weak)] shrink-0">
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <MessageSquareReply size={15} className="text-[var(--crm-accent)] shrink-0" />
              <span className="text-sm font-bold text-[var(--crm-text)] truncate" style={H}>{lead.name}</span>
            </div>
            <p className="text-xs text-[var(--crm-text-2)] mt-1 truncate" style={H}>
              {r?.fromName ? `${r.fromName} · ` : ""}{r?.fromEmail || lead.email}
            </p>
          </div>
          <button onClick={onClose} className="text-[var(--crm-text-3)] hover:text-[var(--crm-text)] transition-colors shrink-0" aria-label="Close">
            <X size={18} />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
          <div className="space-y-1">
            <p className="text-sm font-bold text-[var(--crm-text)]" style={H}>{r?.subject || "(no subject)"}</p>
            {dateStr && <p className="text-xs text-[var(--crm-text-3)]" style={H}>{dateStr}</p>}
          </div>
          {r?.html ? (
            <div className="text-sm text-[var(--crm-text-2)] leading-relaxed crm-reply-html"
            ...
            <pre className="text-sm text-[var(--crm-text-2)] leading-relaxed whitespace-pre-wrap font-sans" style={{ fontFamily: "inherit" }}>
              {r?.text || "(empty reply — no body was captured)"}
            </pre>
          )}
        </div>
        <div className="flex items-center justify-between gap-2 px-5 py-3 border-t border-[var(--crm-border)] shrink-0">
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <MessageSquareReply size={16} className="text-[var(--crm-accent)]" />
          <h2 className="text-sm font-bold text-[var(--crm-text)]" style={H}>Responded</h2>
          {leads && <span className="text-xs text-[var(--crm-text-3)]" style={H}>{leads.length}</span>}
        </div>
      ...
      {error ? (
        ...
        <div className="flex flex-col items-center justify-center gap-3 py-16 crm-surface rounded-2xl text-center px-6">
          <Inbox size={24} className="text-[var(--crm-text-3)]" />
          <p className="text-sm font-bold text-[var(--crm-text-2)]" style={H}>No replies yet</p>
            ...
            return (
              <div key={d.id}
                ...
                <div className="flex items-center gap-2 px-3.5 py-2 bg-[var(--crm-accent-weak)] border-b border-[var(--crm-accent-border)]">
                  <MessageSquareReply size={13} className="text-[var(--crm-accent)] shrink-0" />
                  <span className="text-xs font-bold text-[var(--crm-accent-text)]" style={H}>Responded</span>
                  {d.respondedAt && <span className="text-[11px] text-[var(--crm-accent-text)] opacity-60 ml-auto" style={H}>{relTime(d.respondedAt)}</span>}
                </div>
                <div className="p-3.5 space-y-2">
                  <div className="min-w-0">
                    <p className="text-sm font-bold text-[var(--crm-text)] truncate" style={H}>{d.name}</p>
                  ...
                  {d.reply && (
                    <div className="rounded-xl bg-[var(--crm-bg)] border border-[var(--crm-border)] px-3 py-2.5 space-y-1">
                      {d.reply.subject && <p className="text-xs font-semibold text-[var(--crm-text-2)] truncate" style={H}>{d.reply.subject}</p>}
                      <p className="text-xs text-[var(--crm-text-2)] leading-snug line-clamp-2">{d.reply.snippet || "(no body captured)"}</p>
                    </div>
                  )}
                  ...
                  <div className="flex items-center justify-between gap-2 pt-0.5">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button onClick={() => onSelectLead(d)}
                        className="text-xs font-semibold text-[var(--crm-text-2)] hover:text-[var(--crm-text)] transition-colors" style={H}>
                        Open lead
                      </button>
                    </div>
                  </div>
