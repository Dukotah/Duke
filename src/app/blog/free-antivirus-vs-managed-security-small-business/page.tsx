      ...
      <main>
        ...
        <section className="py-12 bg-white">
          <div className="max-w-2xl mx-auto px-6" style={{ fontFamily: "var(--font-body)" }}>
            ...
            <h3 className="font-bold text-[#18181B] mb-3" style={{ fontFamily: "var(--font-heading)" }}>What free AV covers:</h3>
            <ul className="space-y-2 mb-6">
              {freeAvStrengths.map((item) => (
                <li key={item} className="flex items-start gap-3 text-sm text-[#3F3F46]/70">
            ...
            <h3 className="font-bold text-[#18181B] mb-3" style={{ fontFamily: "var(--font-heading)" }}>What free AV doesn&apos;t cover:</h3>
            ...
            <ul className="space-y-3 mb-6">
              <li className="flex items-start gap-3 text-sm text-[#3F3F46]/70">
                <span className="mt-1.5 w-2 h-2 rounded-full bg-[#F97316] flex-shrink-0" />
                <span><span className="font-semibold text-[#18181B]">MFA on everything:</span> Email, cloud apps, remote access. This one step blocks the large majority of credential-based attacks.</span>
              </li>
              <li className="flex items-start gap-3 text-sm text-[#3F3F46]/70">
                <span className="mt-1.5 w-2 h-2 rounded-full bg-[#F97316] flex-shrink-0" />
                <span><span className="font-semibold text-[#18181B]">Email filtering:</span> A layer between the internet and your inbox that catches phishing, spoofed senders, and malicious attachments before they reach staff.</span>
              </li>
              <li className="flex items-start gap-3 text-sm text-[#3F3F46]/70">
                <span className="mt-1.5 w-2 h-2 rounded-full bg-[#F97316] flex-shrink-0" />
                <span><span className="font-semibold text-[#18181B]">Patching:</span> OS updates and third-party app updates applied promptly. Most exploits target known vulnerabilities with available patches.</span>
              </li>
              <li className="flex items-start gap-3 text-sm text-[#3F3F46]/70">
                <span className="mt-1.5 w-2 h-2 rounded-full bg-[#F97316] flex-shrink-0" />
                <span><span className="font-semibold text-[#18181B]">Tested backups:</span> Not just backups — backups you have actually restored from. An untested backup is a guess.</span>
              </li>
              <li className="flex items-start gap-3 text-sm text-[#3F3F46]/70">
                <span className="mt-1.5 w-2 h-2 rounded-full bg-[#F97316] flex-shrink-0" />
                <span><span className="font-semibold text-[#18181B]">Endpoint protection:</span> Defender or a managed endpoint tool — centralized so you can see what&apos;s happening across every device.</span>
              </li>
              <li className="flex items-start gap-3 text-sm text-[#3F3F46]/70">
                <span className="mt-1.5 w-2 h-2 rounded-full bg-[#F97316] flex-shrink-0" />
                <span><span className="font-semibold text-[#18181B]">A recovery plan:</span> A documented answer to &ldquo;what do we do if ransomware hits on a Tuesday morning?&rdquo; — before you need it.</span>
              </li>
            </ul>
            ...
            <div className="grid sm:grid-cols-2 gap-4 mb-10">
              <div className="rounded-xl border border-[#18181B]/10 p-5 bg-[#FAFAF9]">
                <p className="font-bold text-[#18181B] mb-3 text-sm" style={{ fontFamily: "var(--font-heading)" }}>Stay DIY if:</p>
              ...
              <div className="rounded-xl border border-[#18181B]/10 p-5 bg-[#FAFAF9]">
                <p className="font-bold text-[#18181B] mb-3 text-sm" style={{ fontFamily: "var(--font-heading)" }}>Consider managed security if:</p>
                <ul className="space-y-2">
            ...
            <p className="text-[#3F3F46]/70 mb-4 leading-relaxed">
              Before committing to any managed security service, it&apos;s worth understanding your actual exposure. A flat-fee security audit — not a vendor demo — maps out where you&apos;re covered, where you&apos;re not, and what&apos;s worth fixing first given your budget and risk profile. We offer those audits for Sonoma County businesses at <Link href="/pricing" className="text-[#F97316] hover:underline">$750–$1,200</Link>, and the output is a plain-English report you own, not a locked-in service contract.
            </p>
            <p className="text-[#3F3F46]/70 mb-8 leading-relaxed">
              If you want to know more about what we cover under <Link href="/it-support-sonoma-county" className="text-[#F97316] hover:underline">IT support and managed services</Link>, that page walks through what&apos;s included for Sonoma County teams. The security audit is a good starting point if you&apos;re not sure where you stand.
            </p>

            <div className="mt-10 p-6 rounded-2xl bg-[#18181B] text-white">
              <p className="text-sm font-semibold mb-2 text-[#F97316]" style={{ fontFamily: "var(--font-heading)" }}>Not sure where your gaps are?</p>
