...
function statusColor(s: "pass" | "warn" | "fail") {
  return s === "pass" ? "text-green-400" : s === "warn" ? "text-copper-bright" : "text-red-400";
}
function statusBg(s: "pass" | "warn" | "fail") {
  return s === "pass" ? "bg-green-500/10" : s === "warn" ? "bg-copper/10" : "bg-red-500/10";
}
      ...
      <div className="flex items-center gap-3 px-5 py-4 border-b border-zinc-800/80 bg-gradient-to-b from-white/[0.02] to-transparent">
        ...
        {status === "loading" && (
          <span className="inline-flex items-center gap-1.5 text-zinc-400 text-xs font-medium">
            <Loader2 size={13} className="animate-spin text-copper-bright" />
            Running
          </span>
        )}
        {status === "done" && (
              ...
              <div>
                ...
                {d.opportunities.slice(0, 3).map((o, i) => (
                  <div key={i} className="flex items-center gap-2 py-1.5 border-t border-zinc-800 first:border-0">
                    ...
                    {o.displayValue && <span className="text-copper-bright text-xs font-mono">{o.displayValue}</span>}
                  </div>
                ))}
              </div>
            )}
    ...
    <SectionCard title="SSL Certificate" icon="🔒" status={state.status}>
      ...
      {state.status === "done" && (() => {
        ...
        const expiryColor = !d.error
          ...
          : d.daysUntilExpiry < 30 ? "text-copper-bright"
          : "text-green-400"
          : "text-zinc-400";
        return (
          <div className="space-y-4">
...
function SEOResults({ state }: { state: CheckState<SEOData> }) {
  ...
  const sevColor = (s: SEOIssue["severity"]) => s === "pass" ? "text-green-400" : s === "warning" ? "text-copper-bright" : "text-red-400";
  const sevBg = (s: SEOIssue["severity"]) => s === "pass" ? "bg-green-500/10" : s === "warning" ? "bg-copper/10" : "bg-red-500/10";
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              ...
              <div className="space-y-0.5">
                ...
                <p className="text-zinc-400 text-xs"><span className="text-copper-bright font-semibold">{warnings}</span> warnings</p>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-4 gap-2">
              {[
                ...
                { label: "Redirects", value: d.redirects.length, color: "text-copper-bright" },
              ].map(s => (
            ...
            {d.redirects.length > 0 && (
              <div className="space-y-1">
                <p className="text-copper-bright text-xs font-semibold uppercase tracking-wider mb-2">Redirects</p>
                {d.redirects.map((link, i) => (
                  <div key={i} className="flex items-start gap-2 bg-[#18181B] border border-zinc-800/80 rounded-lg px-3 py-2">
                    <span className="bg-copper/10 text-copper-bright text-[10px] font-bold px-1.5 py-0.5 rounded flex-shrink-0">
                      {link.status}
                    </span>
                    <p className="text-zinc-300 text-xs break-all">{link.url}</p>
                  </div>
...
export default function AuditSuiteTool({
  ...
  // Auto-run when arriving from a deep link (e.g. /audit?url=...) so the
  // homepage AuditTeaser hand-off still fires the audit on load.
  useEffect(() => {
    if (autoRun && initialUrl.trim()) void runAudit(initialUrl);
  ...
  return (
    <div className="text-white">
      {/* Hero */}
      <section className="relative overflow-hidden pt-6 pb-12 px-0 text-center">
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 max-w-xl mx-auto">
            <div className="relative flex-1 group">
              <Globe
                size={18}
                className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-zinc-500 transition-colors group-focus-within:text-copper-bright"
              />
              <input
                type="text"
                inputMode="url"
                ...
                disabled={running}
                className="w-full bg-zinc-900/80 border border-zinc-700 rounded-full pl-11 pr-5 py-3.5 text-[15px] text-white placeholder-zinc-500 transition-all outline-none focus:border-copper/70 focus:ring-4 focus:ring-copper/15 hover:border-zinc-600 disabled:opacity-60"
              />
            </div>
            <button
              type="submit"
              disabled={running || !inputUrl.trim()}
              className="inline-flex items-center justify-center gap-2 bg-copper hover:bg-copper-bright active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-copper disabled:active:scale-100 text-white font-bold px-7 py-3.5 rounded-full transition-all text-sm whitespace-nowrap shadow-lg shadow-copper/20 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-copper/30"
            >
              {running ? (
                <>
          ...
          {running && (
            <p className="flex items-center justify-center gap-2 text-zinc-500 text-xs mt-4">
              <Loader2 size={12} className="animate-spin text-copper-bright" />
            ...
            {auditedUrl && (
              <div className="flex justify-center mb-6">
                <span className="inline-flex items-center gap-2 max-w-full rounded-full border border-zinc-800 bg-zinc-900/70 px-4 py-1.5 text-xs">
                  <Globe size={13} className="shrink-0 text-copper-bright" />
            ...
            {!running && (
              <>
                    ...
                    <>
                      ...
                      <div className="flex flex-col sm:flex-row gap-2">
                        <input
                          ...
                          className="flex-1 bg-zinc-800/80 border border-zinc-700 rounded-full px-4 py-2.5 text-white placeholder-zinc-500 text-sm transition-all outline-none focus:border-copper/70 focus:ring-4 focus:ring-copper/15 hover:border-zinc-600"
                        />
                        <button
                ...
                >
                  ...
                  />
                  <p className="text-copper-bright text-xs font-semibold uppercase tracking-[0.18em] mb-2">Free Consultation</p>
                  ...
                  <Link
                    href="/#contact"
                    className="group inline-flex items-center gap-2 bg-copper hover:bg-copper-bright active:scale-[0.98] text-white font-bold px-8 py-3 rounded-full transition-all text-sm shadow-lg shadow-copper/25 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-copper/30"
                  >
                    Get a Free Review
          ...
          <div className="max-w-3xl mx-auto">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                <div
                  key={item.label}
                  className="group bg-zinc-900/60 border border-zinc-800 rounded-2xl p-5 text-center transition-all duration-200 hover:border-copper/30 hover:bg-zinc-900 hover:-translate-y-0.5 hover:shadow-xl hover:shadow-black/30"
                >
