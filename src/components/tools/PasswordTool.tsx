    ...
    <div className="text-white">
      <section className="pt-6 pb-12 px-6 text-center">
        <div className="max-w-xl mx-auto">
          ...
          <h1 className="text-4xl sm:text-5xl font-black mb-4 leading-tight">
            Password{" "}
            <span className="text-copper-bright">Strength & Breach Checker</span>
          </h1>
          ...
          <p className="text-zinc-500 text-xs">
            Breach check uses <a href="https://haveibeenpwned.com/Passwords" target="_blank" rel="noopener noreferrer" className="text-copper-bright hover:text-copper">Have I Been Pwned</a>&apos;s k-anonymity API — your actual password never leaves your browser.
          </p>
        </div>
      </section>
      ...
      <section className="px-6 pb-24">
        <div className="max-w-lg mx-auto space-y-6">
          {/* Input */}
          <div className="bg-ink-2 border border-hairline rounded-2xl p-6 space-y-4">
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  autoComplete="off"
                  className="w-full bg-ink-1 border border-hairline rounded-xl px-4 py-3.5 pr-12 text-white placeholder-zinc-600 focus:outline-none focus:border-copper transition-colors text-base font-mono"
                />
                <button
                  type="button"
                  ...
                  <ul className="space-y-1">
                    {strength.feedback.map((tip, i) => (
                      <li key={i} className="text-zinc-400 text-xs flex items-start gap-2">
                        <span className="text-copper-bright flex-shrink-0 mt-0.5">→</span>{tip}
                      </li>
                    ))}
                  </ul>
                )}
            ...
            <button
              ...
              className="w-full bg-copper hover:bg-copper-bright disabled:opacity-40 disabled:cursor-not-allowed text-ink-0 font-bold py-3.5 rounded-full transition-colors text-sm"
            >
          ...
          {/* Breach result */}
          {breach && (
            <div className={`rounded-2xl p-6 border ${breach.breached ? "bg-red-500/10 border-red-500/30" : breach.error ? "bg-ink-2 border-hairline" : "bg-green-500/10 border-green-500/30"}`}>
          ...
          {/* How it works */}
          <div className="bg-ink-2 border border-hairline rounded-2xl p-6 space-y-3">
            ...
            <ol className="space-y-2">
              {[
                ...
                <li key={i} className="flex items-start gap-3 text-sm text-zinc-400">
                  <span className="w-5 h-5 rounded-full bg-copper/20 text-copper-bright flex items-center justify-center text-[10px] font-bold flex-shrink-0 mt-0.5">{i + 1}</span>
                  {step}
                </li>
              ))}
            </ol>
          ...
          <div className="bg-ink-2 border border-hairline rounded-2xl p-6 space-y-3">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              {[
                ...
                <div key={i} className="bg-ink-1 rounded-xl p-4">
          ...
          {/* CTA */}
          <div className="rounded-2xl p-6 text-center border border-hairline bg-ink-2">
            ...
            <Link href="/#contact" className="inline-block bg-copper hover:bg-copper-bright text-ink-0 font-bold px-8 py-3 rounded-full transition-colors text-sm">
              Talk to Us
            </Link>
          </div>
        </div>
