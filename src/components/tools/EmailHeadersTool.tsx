...
function authColor(status: AuthStatus) {
  ...
  if (status === "none") return "text-copper-bright bg-copper/10 border-copper/20";
  return "text-zinc-400 bg-ink-3 border-hairline";
}
      ...
      <section className="pt-6 pb-12 px-6 text-center">
        <div className="max-w-2xl mx-auto">
          ...
          <h1 className="text-4xl sm:text-5xl font-black mb-4 leading-tight">
            Email Header{" "}
            <span className="text-copper-bright">Analyzer</span>
          </h1>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                ...
                className="text-copper-bright hover:text-copper text-xs font-semibold transition-colors"
              >
                Load Example
              </button>
            </div>
            <textarea
              ...
              rows={10}
              className="w-full bg-ink-2 border border-hairline rounded-2xl px-5 py-4 text-white placeholder-zinc-600 focus:outline-none focus:border-copper transition-colors text-xs font-mono resize-none"
            />
                    ...
                    {parsed.hops.map((hop, i) => (
                      <div key={i} className="flex items-start gap-3">
                        <div className="flex flex-col items-center flex-shrink-0">
                          <div className="w-6 h-6 rounded-full bg-copper/20 border border-copper/30 flex items-center justify-center text-[10px] font-bold text-copper-bright">{i + 1}</div>
                          {i < parsed.hops.length - 1 && <div className="w-px h-4 bg-ink-3 my-1" />}
                        </div>
                        <div className="flex-1 bg-ink-1 rounded-xl px-4 py-3 mb-2">
                          <p className="text-zinc-300 text-xs font-mono">{hop.from} → {hop.by}</p>
                          {hop.delay !== "—" && i > 0 && (
                            <p className="text-copper-bright text-[11px] mt-1">Delay: {hop.delay}</p>
                          )}
                        </div>
                      </div>
                    ))}
